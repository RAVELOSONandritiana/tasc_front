...
export const actions = {
	create: async ({ request, locals }) => {
		...
		else if (!/^[\w.-]+@[\w.-]+\.\w+$/.test(email)) errors.email = 'Format invalide';
		...
		try {
			await createPersonnel({
				...
				commune: communeResidence.trim(),
				region: regionResidence.trim() || undefined,
				province: provinceResidence.trim() || undefined,
				lieuNaissance: lieuNaissance.trim() || undefined,
				regionNaissance: regionNaissance.trim() || undefined,
