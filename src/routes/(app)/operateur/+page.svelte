<script lang="ts">
	...
	import { IdCard, Plus, X } from '@lucide/svelte/icons';
	import { Spinner } from '$lib/components/ui/spinner';
	import type { ActionResult } from '@sveltejs/kit';

	const { data }: PageProps = $props();

	let listOperateur = $state(data.operateur);

	function supprimerOperateur(id: string) {
		listOperateur = listOperateur.filter((p) => p.id !== id);
	}
	const allPersonnel = $state<Personne[]>((data.personnel || []).map((p) => ({
		id: p.id,
		name: p.name,
		lastname: p.lastname,
			...
			<div class="space-y-4">
				...
				>
					<div class="flex items-center gap-3">
						...
						<div>
							<h1 class="text-xl font-bold tracking-tight">Operateurs</h1>
							<p class="text-xs text-muted-foreground">
								{listFiltered.length} opérateur{listFiltered.length > 1 ? 's' : ''}
							</p>
						</div>
					</div>
					<Button
				...
				<Card class="animate-slide-up stagger-1 p-4 opacity-0">
					<SearchInput bind:value={searchText} placeholder="Rechercher un opérateur" />
				</Card>
			</div>
		</div>
		...
		<div class="p-4 md:p-6">
			{#if listFiltered.length === 0}
				<div class="flex flex-col items-center justify-center py-16 text-muted-foreground">
					<IdCard class="size-12 text-muted-foreground/30" />
					<p class="mt-4 text-sm font-medium">Aucun opérateur enregistré</p>
					<p class="mt-1 text-xs">Ajoutez un opérateur pour commencer.</p>
				</div>
			{:else}
				<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
					{#each listFiltered as op, i (op.id)}
						<div
						...
						>
							<PersonCard
								personne={op}
								role="OPERATEUR"
								detail={`Matricule - ${op.compte?.matricule || '—'}`}
								email={op.email}
								phone={op.phone}
								domicile={op.domicile}
								hrefProfil={`/profil/${op.compte?.id || op.personneId}`}
								onDelete={supprimerOperateur}
							/>
						</div>
					{/each}
				</div>
		...
		<form method="POST" action="?/createFromPersonne" use:enhance={() => {
			...
			return async ({ result }: { result: ActionResult }) => {
				...
				if (result.type === 'success') {
					const newOp = result.data?.result;
					if (newOp?.compte && newOp?.personne) {
						const mappedOp = {
							id: newOp.compte.id,
							name: newOp.personne.name,
							lastname: newOp.personne.lastname,
							domicile: newOp.personne.domicile || '',
							fokontany: newOp.personne.fokontany || '',
							commune: newOp.personne.commune || '',
							phone: newOp.personne.phone,
							email: newOp.personne.email,
							imageUrl: newOp.personne.imageUrl || null,
							compte: newOp.compte,
							personneId: newOp.personne.id
						};
						listOperateur = [...listOperateur, mappedOp];
					}
					success = true;
			...
			<Dialog.Header class="mb-1 space-y-1">
				<Dialog.Title class="text-xl font-semibold">Ajouter un opérateur</Dialog.Title>
				<Dialog.Description>
					Recherchez un personnel existant et associez-lui un profil opérateur.
				</Dialog.Description>
			</Dialog.Header>
			...
			<div class="space-y-4 py-2">
				...
				{#if selectedPersonne}
					<div class="grid gap-2">
						...
						<Input id="matricule" name="matricule" bind:value={matricule} placeholder="Ex: OPE-047" />
					</div>
				{/if}
			</div>
